<%@ Page Language="VB" AutoEventWireup="false" CodeFile="bulletchat1.aspx.vb" Inherits="bulletchat1" %>
<!DOCTYPE html>
...
<head runat="server">
    ...
    .left {
      ...
      white-space: nowrap;
      user-select: none;
      transition: transform 30s linear; /* 时间相同 越长的弹幕滑动距离越长 所以越快~ */
    }
    h1 {
  position: relative;
...
h2::after {
  ...
  mix-blend-mode: multiply;
    ...
    <script src="js/barrage1.js"></script>
...
<div id="outside">
        <div id="wrapper"></div>
        <span style="display: inline-block;padding-left: 380px;" id="new"  ></span>
        <span style="display: inline-block;float:right;padding-right: 780px;"  id="new2"  ></span></div>
...
$('#wrapper').barrage({
    ...
    // Update the comments and display them in the console
    let a = recipientNames[0];
    let b = messageTexts[0];
    let c = senderNames[0];
    document.getElementById("new").innerHTML = '<center><div class="barrage-item" ><h2 type="text" id="text" style="text-align:center;max-width: 270px;" value="' + '">' +'<div style="text-align: left;">To:'+a+'</div><br>'+ b+'<br>' +'<div style="text-align: right;">'+c+ '</div></h2></div></center>';

    let d = recipientNames[1];
    let e = messageTexts[1];
    let f = senderNames[1];
    document.getElementById("new2").innerHTML = '<center><div class="barrage-item"><h2 type="text" id="text" style="text-align:center;max-width: 270px;" value="' + '">' +'<div style="text-align: left;">To:'+d+'</div><br>'+ e+'<br>' +'<div style="text-align: right;">'+f+ '</div></h2></div></center>';
},
        error: function () {
